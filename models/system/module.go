...
type Module struct {
	Id         int64 `orm:"pk;column(module_id);"`
	Userid     int64
	Name       string
	Aliasname  string
	Started    int64
...
func GetProject(id int64) (Module, error) {
	var project Module
	var err error
...
func UpdateProject(id int64, updPro Module) error {
	var pro Module
	o := orm.NewOrm()
	pro = Module{Id: id}
...
func AddProject(updPro Module) error {
	o := orm.NewOrm()
	pro := new(Module)
...
//项目列表
func ListProject(condArr map[string]string, page int, offset int) (num int64, err error, user []Module) {
	o := orm.NewOrm()
	o.Using("default")
...
func ListMyProject(userId int64, page int, offset int) (num int64, err error, ops []Module) {
	if page < 1 {
		page = 1
	}
	if offset < 1 {
