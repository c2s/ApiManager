...
func UpdateGooutsApprover(id int64, upd GooutsApprover) error {
	...
	if err == nil {
		...
		if upd.Status == 2 {
			...
			o.Raw("UPDATE am_goouts_approver SET status = ?,summary = ?, changed = ? WHERE gooutid = ? AND approverid != ?", 2, "前审批人拒绝，后面审批人默认为拒绝状态", time.Now().Unix(), upd.Gooutid, id).Exec()
		} else {
...
func ListGooutApproverProcess(gooutid int64) (num int64, err error, user []GooutApproverProcess) {
	...
	qb.Select("upr.userid", "upr.realname", "p.name AS position", "u.avatar", "la.status", "la.summary", "la.changed").From("am_goouts_approver AS la").
